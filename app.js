(() => {
  ...
  let frames=0, fpsStarted=performance.now(), dirty=true, liveConnected=false, polling=false, lastDepth=null;
  ...
  function setSensorStatus(connected,message){liveConnected=connected;$("#statusText").textContent=message;$("#statusDot").classList.toggle("offline",!connected);}
  async function pollDepth(){if(polling)return;polling=true;try{if(!controls.liveSensor.checked){setSensorStatus(false,"Demo field");return;}const response=await fetch(`/api/depth?t=${Date.now()}`,{cache:"no-store"});if(!response.ok){setSensorStatus(false,"Waiting for Kinect");return;}lastDepth=new Uint16Array(await response.arrayBuffer());const {near,far}=depthBounds(),gamma=+controls.gamma.value/100,keep=+controls.smoothing.value/100,mix=1-keep;for(let i=0;i<grid.values.length&&i<lastDepth.length;i++){const depth=lastDepth[i];if(depth>0&&depth<=far+200){let elevation=Math.max(0,Math.min(1,(far-depth)/(far-near)));elevation=Math.pow(elevation,1/gamma);grid.values[i]=liveConnected&&grid.values[i]>=0?grid.values[i]*keep+elevation*mix:elevation;}else grid.values[i]=-1;}setSensorStatus(true,"Kinect live");dirty=true;}catch(_){setSensorStatus(false,"Server unavailable");}finally{polling=false;}}
  ...
  $("#autoRange").addEventListener("click",autoFit);$("#collapseControls").addEventListener("click",()=>$("#controlsPanel").classList.add("collapsed"));$("#showControls").addEventListener("click",()=>$("#controlsPanel").classList.remove("collapsed"));
  ...
  loadSettings();if(!controls.theme.value)controls.theme.value="earth";updateReadouts();applyTheme();resetTerrain();resize();setInterval(pollDepth,50);pollDepth();requestAnimationFrame(draw);
})();
